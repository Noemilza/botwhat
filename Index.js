...
const start = (kill = new Client()) => {
    console.log(color('\n[DEV]', 'red'), color('- maumau - Família THC <-> +55 92 99215-2454 <-> https://chat.whatsapp.com/H53MdwhtnRf7TGX1VJ2Jje'))
	console.log(color('[malandro]', 'red'), color('Minha inicialização foi concluída, você pode usar agora...\n'))
        ...
        kill.onIncomingCall(async (call) => {
            await kill.sendText(call.peerJid, `Que pena! Chamadas não são suportadas e atrapalham muito! 😊\nTe bloqueei para evitar novas, contate o dono wa.me/92992152454${config.owner.replace('c.us', '')} para efetuar o desbloqueio. 👋`)
